...
class User
{
	...
	jump()
	{
		...
		function _jump()
		{
			user.y = user.jumpV*t - 1/2 * 30 * t**2;
			t+=0.05;
			if (user.y < 0)
			{
				arrowUp = false;
...
const imgGroundArr = ["background/눈기둥(지형).png","background/사막기둥(지형).png","background/풀기둥(지형).png","background/구름(지형).png"];
...
class Ground
{
	...
	draw()
	{
		...
		groundImg.src=this.url;
		if(user.x <= 3200-640 && user.x >= -3200+640)
		{
			ctxGround.drawImage(groundImg, this.x1-user.x, this.y+user.y+312);
		}
		else
		{
			if (user.x > 3200-640)
			{
				ctxGround.drawImage(groundImg, this.x1-3200+640, this.y+user.y+312);
			}
			else if (user.x < -3200+640)
			{
				ctxGround.drawImage(groundImg, this.x1+3200-640, this.y+user.y+312);
...
function makeGround() {
	...
	while(true)
	{
		...
		{
			xArr[i] = Math.floor(Math.random()*6400-360-3200);
		}
		for(let i=0; i<xArr.length; i++)
		{
			for(let m=0; m<xArr.length-i; m++)
			{
				...
				else if((xArr[i] - xArr[xArr.length-m-1]) < 360+410 && (xArr[i] - xArr[xArr.length-m-1]) > -360-410 )
				{
					t=false;
				}
				else
	...
	for ( let i = 0; i < xArr.length; i++ )
	{
		eval("ground"+i+"= new Ground("+xArr[i]+",0,'"+imgGroundArr[Math.floor(Math.random()*2+1)]+"')");
