...
class User
{
	...
	jump(startY)
	{
		...
		function _jump()
		{
			...
			gravityUp();
			if ( user.jumpV < 1/2 * 50 * t && boardingKey == true)
			{
				level=1;
				arrowUp = false;
				user.y = groundY;
...
function gravityUp() {
	...
	for (let i=0; i<groundArr.length;i++)
	{
		if (user.x >= groundArr[i].x1 && user.x <= groundArr[i].x2 && user.y>=groundArr[i].y-15 && user.y<=groundArr[i].y+15)
		{
			groundY = groundArr[i].y;
	...
	if (level == 1)
	{
		for (let i=0; i<cloudArr.length;i++)
		{
			if (user.x >= cloudArr[i].x1 && user.x <= cloudArr[i].x2 && user.y>=cloudArr[i].y-15 && user.y<=cloudArr[i].y+15)
			{
				console.log("저장")
				groundY = cloudArr[i].y+50;
				myGround[0]=groundArr[i].x1;
				myGround[1]=groundArr[i].x2;
				boardingKey=true;
			}
...
function Down() {
	// 땅바닥에 있거나 점프중인 상황 제외
	if (myGround[0] ==false || arrowUp == true )
	{
	}
	else
	{
		// 지형 좌표 밖으로 이동 시점
		if ( myGround[0]>user.x || myGround[1]<user.x )
		{
			console.log("다운")
			user.down();
