...
class projectil_Sh{
	constructor(fx,fy,prot_Lmoving,prot_Rmoving,prot_upmoving,isFired,isHitted,missilePower,project_n){
	...
	projtil_shting(){
		...
		for (let i=0;i<this.project_n; i++){
				//for (let i=0; i<1;i+=i) {
				// c=0;	
				// projtil_array=;
				// fx1=user.x;
...
let fx1=0;
let fy1=0;
let projectil_1 =new projectil_Sh(fx1,fy1,false,false,false,false,false,false,1)
...
{
	let cat;
	if (projtil_array_x[i]<-1450)
	eval(""+i+"= new Ground("+projtil_array_x[i]+",'"+cat+"')");
		// cat = imgGroundArr[1];
	else if (projtil_array_x[i]<1250 && projtil_array_x[i]>=-1450)
	eval("prjR"+i+"= new Ground("+projtil_array_x[i]+",'"+cat+"')");
		// cat = imgGroundArr[2];
	else
		// cat = imgGroundArr[0];
	eval("prjR"+i+"= new Ground("+projtil_array_x[i]+",'"+cat+"')");
