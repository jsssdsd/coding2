...
function createMonster() {
	...
	if (createMyPer <= createPer)
	{
		...
		if (min >=3)
		{
			...
			{
				eval("monster"+monsterCnt+"= new Monster("+xArr+","+0+","+(user.speed)+","+3+
				","+1+","+50+",'"+"github"+"')")
				monsterArr[monsterCnt]=eval("monster"+monsterCnt);
				monsterCnt += 1;
			}
...
let hpD= document.getElementById("hp1");
let hpD2 = hpD.value
let hpD3 = hpD.max
	...
	for (let i = 0; i < monsterArr.length; i++)
	{
		...
		if ( user.x - monsterArr[i].x <= 30 && user.x - monsterArr[i].x >= -30)
		{
			if ( user.y - monsterArr[i].y < 60 && user.y - monsterArr[i].y > -60  )
			{
				...
				monsterArr[i].x = undefined;
				monsterArr[i].y = undefined;
				console.log('hpD2값',hpD2);
				hpD2=hpD2-String(1);
				console.log('hpD2값',hpD2);
				// document.getContext("<progress id='hp1' max= '100'; value='hpD2'></progress>")
				hpD2.innerHTML -="1";
				hpD3.innerHTML ="10";
				// myDiv.write("<progress id='hp1' max= '100'; value='hpD2'></progress>")
...
function drawScoreText() {
	scoreText = '점수: ' + user.score;
	ctxTimeScore.fillText(scoreText, 1100, 150);
}
...
function moveToGameScreen(){
	...
	button1.style.display='block'; // 버튼 표시
...
function loadEn (){
	...
	if(user.life=0) {
		...
		window.textContent("location.href ='ending1.html'");
	}
...
// <button id="S" type="button" class="btn btn-primary mt-5 S">체력바</button>
// <Br>
// <a>HP</a>
