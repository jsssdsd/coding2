...
// let hpV = document.getElementById('hp1').value;
// let hpM = document.getElementById('hp1').max;
// let progress =document.getElementsByTagName('progress');
// progress.setAttribute()
// progress.setAttribute('data-label', user.life);
...
function hpbar()
{
	for (let i=user.life; i>0;i--){
		
	// ctx.fillRect(550,5,900, 20-(5*i));
	// ctx.clearRect(0,0,canvas.width,canvas.height);
	ctx.save();
	ctx.fillStyle = 'red';
	ctx.fillRect(850,5, 40*user.life, 30);

	
	ctx.fillRect(850,5, 400-(user.life ) , 30);
	ctx.fill();
	// ctx.restore();
	// }
	}
